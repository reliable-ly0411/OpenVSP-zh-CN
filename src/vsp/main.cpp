...
int main( int argc, char** argv )
{
    ...
    // The legacy OpenVSP startup check uses libxml2 NanoHTTP to POST usage
    // information and fetch latest_version3.html over plain HTTP.  The
    // official site now redirects to HTTPS and the version file no longer
    // exists, which makes first launch print "Unknown IO error".  Keep the
    // upstream implementation above for traceability, but do not invoke the
    // obsolete network/telemetry path in the localized build.
#if defined( VSP_ENABLE_LEGACY_ONLINE_VERSION_CHECK ) && VSP_ENABLE_LEGACY_ONLINE_VERSION_CHECK
