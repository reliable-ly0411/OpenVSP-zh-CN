...
string VSPAEROMgrSingleton::ComputeGeometry()
{
    ...
    string mesh_geom_id = veh->CompGeomAndFlatten( set, halfFlag, intSubsFlag, degenset, false /*hideset*/, true /*suppressdisks*/, m_UseMode(), m_ModeID, m_NRef(), sub_vec, m_SingleGeomID );
...
string VSPAEROMgrSingleton::CreateSetupFile()
{
    ...
    fprintf( case_file, "Sref = %.17g \n", m_Sref() );
    fprintf( case_file, "Cref = %.17g \n", m_cref() );
    fprintf( case_file, "Bref = %.17g \n", m_bref() );
    fprintf( case_file, "X_cg = %.17g \n", m_Xcg() );
    fprintf( case_file, "Y_cg = %.17g \n", m_Ycg() );
    fprintf( case_file, "Z_cg = %.17g \n", m_Zcg() );
    ...
    {
        fprintf( case_file, "%.17g, ", machVec[i] );
    }
    fprintf( case_file, "%.17g \n", machVec[i++] );

    // Alpha vector
    fprintf( case_file, "AoA = " );
    for ( i = 0; i < alphaVec.size() - 1; i++ )
    {
        fprintf( case_file, "%.17g, ", alphaVec[i] );
    }
    fprintf( case_file, "%.17g \n", alphaVec[i++] );
    ...
    {
        fprintf( case_file, "%.17g, ", betaVec[i] );
    }
    fprintf( case_file, "%.17g \n", betaVec[i++] );
    ...
    {
        fprintf( case_file, "%.17g, ", recrefVec[i] );
    }
    fprintf( case_file, "%.17g \n", recrefVec[i++] );

    fprintf( case_file, "Vinf = %.17g \n", m_Vinf() );

    if ( m_ManualVrefFlag() )
    {
        fprintf( case_file, "Vref = %.17g \n", m_Vref() );
        fprintf( case_file, "Mref = %.17g \n", m_Machref() );
    }

    fprintf( case_file, "Rho = %.17g \n", m_Rho() );
    fprintf( case_file, "StallModel = %d \n", m_StallModel() );
    fprintf( case_file, "Clo2D = %.17g \n", m_Clo2D() );
    fprintf( case_file, "CLMax2D = %.17g \n", m_CLMax2D() );
    ...
    fprintf( case_file, "WakeRelax = %.17g \n", m_WakeRelax() );

    fprintf( case_file, "ForwardGMRESConvergenceFactor = %.17g \n", m_ForwardGMRESConvergenceFactor() );
    fprintf( case_file, "AdjointGMRESConvergenceFactor = %.17g \n", m_AdjointGMRESConvergenceFactor() );
    fprintf( case_file, "NonLinearConvergenceFactor = %.17g \n", m_NonLinearConvergenceFactor() );
    fprintf( case_file, "CoreSizeFactor = %.17g \n", m_CoreSizeFactor() );
    fprintf( case_file, "FarAway = %.17g \n", m_FarAway() );
    ...
    if ( m_PropBladesMode() != vsp::VSPAERO_PROP_STATIC )
    {
        ...
        {
            fprintf( case_file, "NumberOfTimeSteps = %d \n", m_NumTimeSteps.Get() );
            fprintf( case_file, "TimeStep = %.17g \n", m_TimeStepSize() );
        }
...
void RotorDisk::Write_STP_Data( FILE *InputFile ) const
{
    ...
    fprintf( InputFile, "%.17g %.17g %.17g \n", m_XYZ.x(), m_XYZ.y(), m_XYZ.z() );

    fprintf( InputFile, "%.17g %.17g %.17g \n", m_Normal.x(), m_Normal.y(), m_Normal.z() );

    fprintf( InputFile, "%.17g \n", m_Diameter() / 2.0 );
...
void ControlSurfaceGroup::Write_STP_Data( FILE *InputFile )
{
    ...
    {
        fprintf( InputFile, "%.17g, ", m_DeflectionGainVec[i]->Get() );
    }
...
int UnsteadyGroup::WriteGroup( FILE *group_file )
{
    ...
    fprintf( group_file, "RotorDiameter = %.17g\n", m_RotorDia() );
    fprintf( group_file, "OVec = %.17g %.17g %.17g\n", m_Ox(), m_Oy(), m_Oz() );
    fprintf( group_file, "RVec = %.17g %.17g %.17g\n", m_Rx(), m_Ry(), m_Rz() );
    fprintf( group_file, "Velocity = %.17g %.17g %.17g\n", m_Vx(), m_Vy(), m_Vz() );
    fprintf( group_file, "Acceleration = %.17g %.17g %.17g\n", m_Ax(), m_Ay(), m_Az() );
    fprintf( group_file, "Omega = %.17g\n", omega );
    fprintf( group_file, "Mass = %.17g\n", m_Mass() );
    fprintf( group_file, "Ixx = %.17g\n", m_Ixx() );
    fprintf( group_file, "Iyy = %.17g\n", m_Iyy() );
    fprintf( group_file, "Izz = %.17g\n", m_Izz() );
    fprintf( group_file, "Ixy = %.17g\n", m_Ixy() );
    fprintf( group_file, "Ixz = %.17g\n", m_Ixz() );
    fprintf( group_file, "Iyz = %.17g\n", m_Iyz() );
