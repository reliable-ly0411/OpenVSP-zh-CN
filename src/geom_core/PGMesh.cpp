...
bool PGNode::FaceRemoveNodeTest( PGFace* f ) const
{
    vec3d nvec = f->m_Nvec;
...
void PGFace::WalkManifoldRegion() const
{
    for ( int i = 0; i < m_EdgeVec.size(); i++ )
    {
        PGEdge *e = m_EdgeVec[i];
...
void PGMesh::RemoveEdgesMergeFaces( vector < PGEdge* > edges, void ( * facemergeproperties ) ( PGFace *f0, PGFace *f1 ) )
{
    ...
    if ( !edges.empty() && manifoldedges )
    {
        ...
        if ( i0 >= 0 && i1 >= 0 )
        {
            ...
            vector_remove_vector( ev0, edges );
            vector_remove_vector( ev1, edges );
...
void PGMesh::MakeManifoldRegions()
{
    m_Regions.clear();
...
void PGMesh::BuildFromTMesh( const TMesh* tmi )
{
    ...
    m_PGMulti->m_CopyIndxVec = SubSurfaceMgr.m_CompCopyIndex;
...
void PGMesh::BuildFromPGMesh( const PGMesh* pgm )
{
    ...
    m_PGMulti->m_CopyIndxVec = pgm->m_PGMulti->m_CopyIndxVec;
...
int PGMulti::GetCopyIndex( int part ) const
{
    return m_CopyIndxVec[ part - 1 ];
}
