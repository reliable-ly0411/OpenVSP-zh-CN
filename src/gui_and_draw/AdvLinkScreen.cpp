...
AdvLinkScreen::AdvLinkScreen( ScreenMgr* mgr ) : BasicScreen( mgr, 829, 800, "Advanced Parameter Links", "AdvLink.html" )
{
    ...
    ( dynamic_cast <Vsp_Group*> ( m_InputGroup.GetGroup() ) )->SetAllowDrop( true );
    m_InputGroup.GetGroup()->callback( staticScreenCB, this );

    ( dynamic_cast <Vsp_Group*> ( m_OutputGroup.GetGroup() ) )->SetAllowDrop( true );
                ...
                if ( veh->m_AdvLinkGenDefName() )
                {
                    ...
                    if ( p )
                    {
                        ...
                        if ( edit_link->DuplicateVarName( name ) )
                        {
                            ...
                            if ( 0 == fl_choice( "%s 不是唯一名称，是否改用 %s？", VSPTranslate( "Cancel" ).c_str(), "采用建议",  0, name.c_str(), new_name.c_str() ) )
                            {
                                cancel = true;
                            }
                            name = new_name;
