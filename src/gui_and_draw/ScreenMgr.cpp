...
void ScreenMgr::MessageCallback( const MessageBase* from, const MessageData& data )
{
    ...
    else if ( data.m_String == string( "Error" ) )
    {
        ...
        const string translated_message = VSPTranslate( message );
        const char* msg = translated_message.c_str();
...
string ScreenMgr::NativeFileChooser( const string &title, const string &filter, int mode, const string &preset_fname, const string &dir )
{
    ...
    const string translated_title = VSPTranslate( title );
    m_NativeFileChooser->title( translated_title.c_str() );
...
{
    const string translated_message = VSPTranslate( message );
    char *buf = new char[ translated_message.size() + 1 ];
    memcpy( buf, translated_message.data(), translated_message.size() );
    buf[translated_message.size()] = '\0';
