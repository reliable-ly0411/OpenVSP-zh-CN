...
ParmLinkScreen::ParmLinkScreen( ScreenMgr* mgr ) : BasicScreen( mgr, 600, 615 + 100, "Parm Link: ( A * Scale + Offset = B )" )
{
    ...
    m_GenLayout.AddYGap();
    m_LinkAttrEditor.Init( &m_GenLayout, m_FLTK_Window, this , staticScreenCB, true , m_GenLayout.GetY(), m_GenLayout.GetRemainY() );
}
...
void ParmLinkScreen::GuiDeviceCallBack( GuiDevice* device )
{
    ...
    else if( device == &m_AddLink )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", VSPTranslate( "Error: Identical Parms or Already Linked" ).c_str() );
        }
        m_SelectedLinks.clear();
    }
    ...
    else if ( device == &m_LinkConts )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", VSPTranslate( "Error: Identical Comps" ).c_str() );
        }
        m_SelectedLinks.clear();
    }
    else if ( device == &m_LinkGroups )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", VSPTranslate( "Error: Identical Group" ).c_str() );
        }
        m_SelectedLinks.clear();
    }
