...
{
    // Localize FLTK's built-in message-dialog buttons. These labels are used
    // by fl_message/fl_alert and otherwise bypass the OpenVSP widget helpers.
    fl_no = "否";
    fl_yes = "是";
    fl_ok = "确定";
    fl_cancel = "取消";
    ...
    m_LocalizationAboutMenuItem.Init( this, m_MenuBar, "Help/About Codex AI Localization..." );
    ...
    m_FileNameBox = new Fl_Box(m_GenLayout.GetX(), m_GenLayout.GetY(), filew, 20, "文件名：");
    ...
    const string localized_version = string( VSPVERSION3 ) + " | Codex AI 生成汉化版";
    m_TitleBox->copy_label( localized_version.c_str() );
...
void MainVSPScreen::BoldEntries( const string &mpath )
{
    ...
    const string translated_path = VSPTranslateMenuPath( mpath );
    item = (Fl_Menu_Item*) m_MenuBar->find_item( translated_path.c_str() );
    if ( item )
    {
...
void MainVSPScreen::SetFileLabel( const string &fname )
{
    ...
    label.append( fname );
    m_FileNameBox->copy_label( VSPTranslate( label ).c_str() );

    // In menu bar, file name only.
    string title = string( VSPVERSION2 ) + " [Codex AI 汉化版]     " + GetFilename( fname );
