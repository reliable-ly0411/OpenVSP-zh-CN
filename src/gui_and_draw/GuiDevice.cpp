...
void ParmButton::Update( const string& parm_id )
{
    ...
    if( m_ButtonNameUpdate )
    {
        ...
        if ( parm_ptr )
        {
            m_Button->copy_label( VSPTranslate( parm_ptr->GetName() ).c_str() );
...
{
    m_Button->copy_label( VSPTranslate( name ).c_str() );
    DebugLabelSize( m_Button );
}
...
void TriggerButton::SetLabel( const char* label )
{
    if ( m_Button )
    {
        m_Button->copy_label( VSPTranslate( label ).c_str() );
    }
}
...
void Choice::UpdateItems( bool keepsetting )
{
    ...
    if ( m_Choice )
    {
        ...
        for ( int i = 0 ; i < ( int )m_Items.size() ; i++ )
        {
            ...
            m_Choice->replace( i, VSPTranslate( m_Items[i] ).c_str() );
        }
...
void ParmTreePicker::AddParmEntry( const string &ParmID, ContainerTreeData &conttree, Fl_Tree_Item* groupitem )
{
    ...
    td.m_Button->copy_label( VSPTranslate( p->GetName() ).c_str() );
...
void PCurveEditor::Update()
{
    if ( m_Curve && m_canvas )
    {
        ...
        if ( m_CurveB )
        {
            ...
            m_canvas->m_Ysecondary->copy_label( VSPTranslate( m_LabelB ).c_str() );
        ...
        if ( !m_FreezeAxis )
        {
            ...
            m_canvas->current_x()->copy_label( VSPTranslate( m_Curve->GetXDsipName() ).c_str() );
            m_canvas->current_y()->copy_label( VSPTranslate( m_Curve->GetYDsipName() ).c_str() );
        }
